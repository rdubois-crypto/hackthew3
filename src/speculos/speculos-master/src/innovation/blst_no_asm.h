...
#define LSHIFT_MOD_IMPL(bits)                                                  \
  inline void lshift_mod_##bits(vec##bits ret, const vec##bits a,              \
                                size_t count, const vec##bits p)               \
  {                                                                            \
...
static void from_mont_n(limb_t ret[], const limb_t a[], const limb_t p[],
                        limb_t n0, size_t n)
{
  llimb_t limbx;
...
static void redc_mont_n(limb_t ret[], const limb_t a[], const limb_t p[],
                        limb_t n0, size_t n)
{
  llimb_t limbx;
...
#define RSHIFT_MOD_IMPL(bits)                                                  \
  inline void rshift_mod_##bits(vec##bits ret, const vec##bits a,              \
                                size_t count, const vec##bits p)               \
  {                                                                            \
...
#endif
static limb_t
lshift_2(limb_t hi, limb_t lo, size_t l)
{
  size_t r = LIMB_T_BITS - l;
  limb_t mask = 0 - (is_zero(l) ^ 1);
  return (hi << (l & (LIMB_T_BITS - 1))) |
         ((lo & mask) >> (r & (LIMB_T_BITS - 1)));
}

/*
 * https://eprint.iacr.org/2020/972 with 'k' being LIMB_T_BITS-1.
 */
static void ab_approximation_n(limb_t a_[2], const limb_t a[], limb_t b_[2],
                               const limb_t b[], size_t n)
{
