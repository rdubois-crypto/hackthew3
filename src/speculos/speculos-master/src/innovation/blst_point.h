...
#define DECLARE_POINT(ptype, bits)                                             \
  ...
  static void ptype##_from_Jacobian(ptype *out, const ptype *in);              \
                                                                               \
  static inline void ptype##_cswap(ptype *restrict a, ptype *restrict b,       \
                                   bool_t cbit)                                \
  {                                                                            \
    vec_cswap(a, b, sizeof(ptype), cbit);                                      \
  }                                                                            \
  static inline void ptype##_ccopy(ptype *restrict a, const ptype *restrict b, \
                                   bool_t cbit)                                \
  {                                                                            \
...
#define DECLARE_PRIVATE_POINTXZ(ptype, bits)                                   \
  ...
  static void ptype##xz_ladder_step(ptype##xz *r, ptype##xz *s,                \
                                    const ptype##xz *p);                       \
  static void ptype##xz_ladder_post(ptype *ret, const ptype##xz *r,            \
                                    const ptype##xz *s, const ptype##xz *p,    \
                                    const vec##bits Y1);                       \
                                                                               \
