...
#define POINT_MULT_SCALAR_WX_IMPL(ptype, SZ)                                   \
  static void ptype##_gather_booth_w##SZ(                                      \
      ptype *restrict p, const ptype table[1 << (SZ - 1)], limb_t booth_idx)   \
  {                                                                            \
    size_t i;                                                                  \
    ...
    for (i = 2, j = 1; i < 1 << (SZ - 1); i += 2, j++)                         \
      ptype##_add(&row[i], &row[j], &row[j - 1]), /* row[2]=p*(2+1) */         \
          ptype##_double(&row[i + 1], &row[j]);   /* row[3]=p*(2+2) */         \
                                                                               ...
                                                                               \
  static void ptype##s_mult_w##SZ(ptype *ret, const ptype *points[],           \
                                  size_t npoints, const byte *scalars[],       \
                                  size_t bits, ptype table[][1 << (SZ - 1)])   \
  {                                                                            \
    limb_t wmask, wval;                                                        \
    ...
    if (table == NULL)                                                         \
      table = (ptype(*)[1 << (SZ - 1)])                                        \
          alloca((1 << (SZ - 1)) * sizeof(ptype) * npoints);                   \
                                                                               \
    if (points != NULL) {                                                      \
